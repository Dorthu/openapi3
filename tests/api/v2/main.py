...
@router.post('/pet',
             ...
             responses={201: {"model": schema.Pet},
                     409: {"model": schema.Error}}
             )
def createPet(response: Response,
    ...
    if pet.name in ZOO:
        return JSONResponse(status_code=starlette.status.HTTP_409_CONFLICT,
                            content=schema.Error(code=errno.EEXIST,
...
def deletePet(response: Response,
              pet_id: int = Query(..., alias='petId')) -> schema.Pets:
    for k, v in ZOO.items():
        if pet_id == v.identifier:
            del ZOO[k]
