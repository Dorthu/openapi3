...
not310 = pytest.mark.skipif(sys.version_info >= (3, 10, 0), reason="tatsu 5.7 requires 3.10, we rely on 5.6")
...
def test_get(httpx_mock, param, result):
    ...
    httpx_mock.add_response(
        headers={"Location": "http://example.org/subscription/1"},
    )
    client = httpx.Client()
    resp = client.post(
        url, json=data, headers={"Location": "http://example.org/subscription/1", "Content-Type": "application/json"}
    )
