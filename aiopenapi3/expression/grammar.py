...
def loads(data, trace=False, colorize=False):
    from .model import (
        RuntimeExpressionModelBuilderSemantics,
        RuntimeExpression,
        Expression,
        JSONPointer,
        Body,
    ...
    model = parser.parse(
        ...
        semantics=RuntimeExpressionModelBuilderSemantics(
            types=[RuntimeExpression, Expression, JSONPointer, Body, Header, Query, Path]
        ),
        filename="…",
    )
    return model
